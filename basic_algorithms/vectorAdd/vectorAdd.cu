...
void verify_result(std::array<float, 2048> &a, std::array<float, 2048> &b, std::array<float, 2048> &c)
{
...
int main()
{
    ...
    constexpr size_t array_size = sizeof(float)*NUM_ELEM;
    // Allocate memory on host
    std::array<float, NUM_ELEM> h_a, h_b, h_c;
